...
func _check_editions() -> int:
	...
	var r1: Dictionary = Scoring.score([foil], null)     # 5+5+15 = 25 chips, mult 1 -> 25
	var holo := _c(5, Aspects.Id.LIFE)
	holo.edition = CardData.Edition.HOLO
	var r2: Dictionary = Scoring.score([holo], null)     # 10 chips, mult 1+2 = 3 -> 30
	var poly := _c(5, Aspects.Id.LIFE)
	poly.edition = CardData.Edition.POLYCHROME
	var r3: Dictionary = Scoring.score([poly], null)     # 10 chips, mult 1*1.3 -> 13
